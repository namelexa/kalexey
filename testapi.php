...
class MagentoAPI
{
    ...
    public function getAllSampleRequests()
    {
        ...
        $requestUrl = $this->_magentoBaseUrl . $this->_magentoApiController . 'askQuestion/search?'.$searchCriteriaQuery;
        var_dump($requestUrl);
    ...
    public function getSampleRequest($askQuestionId)
    {
        if (!$this->_token) {
            return false;
        }
        $headers = array("Authorization: Bearer $this->_token");
        $requestUrl = $this->_magentoBaseUrl . $this->_magentoApiController . 'askQuestion/'.$askQuestionId;
        var_dump($requestUrl);
        $ch = curl_init($requestUrl);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public function saveSampleRequest($askQuestion)
    {
        if (!$this->_token) {
            return false;
        }
        $requestUrl = $this->_magentoBaseUrl . $this->_magentoApiController . 'askQuestion';
        if ($askQuestion->id) {
            $requestUrl .= '/'.$askQuestion->id;
        }
        var_dump($requestUrl);
